...
const Detail3 = () => {
    ...
    useEffect(() => {
        const fetchLeave = async () => {
            try {
                const response = await axios.get(`https://fatp-api.onrender.com/api/leave3/detail/${id}`, {
                    headers: {
    ...
    const changeStatus = async (id, status) => {
        try {
            const response = await axios.put(`https://fatp-api.onrender.com/api/leave3/${id}`, { status, approver: selectedApprover  }, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
            ...
            if (response.data.success) {
                ...
                navigate('/admin-dashboard/leaves3');
            }
        } catch (error) {
            if (error.response && !error.response.data.success) {
                alert(error.response.data.error);
                        ...
                        <div className='flex flex-col space-y-4'>
                            ...
                            {leave.rawMaterialStorage && (
                                <div className="border-t border-b pt-4 pb-4">
                                    <h3 className="text-lg font-bold mb-4">DAILY MAINTENANCE CHECKLIST  PDL TESTER</h3>
